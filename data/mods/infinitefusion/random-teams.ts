...
export class RandomTeams {
	...
	randomSet(
		...
		let headspecies =this.dex.species.get(this.prng.sample(pokemonPool))
		...
		const headsets = (this as any)[`random${isDoubles ? 'Doubles' : ''}Sets`][headspecies.id]["sets"];
		const possibleSets = [];
		...
		const headset = this.sampleIfArray(possibleSets);
		const role = set.role;
		const movePool: string[] = [];
		for (const movename of set.movepool) {
			movePool.push(this.dex.moves.get(movename).id);
		}
		for (const movename of headset.movepool) {
			movePool.push(this.dex.moves.get(movename).id);
		}
		...
		const types = species.types;
		const abilities = new Set(Object.values(species.abilities).concat(Object.values(headspecies.abilities)));
		...
		// Get level
		const level = Math.floor((this.getLevel(species, isDoubles)+this.getLevel(headspecies,isDoubles))/2);
		...
		while (baseSpeciesPool.length && pokemon.length < this.maxTeamSize) {
			...
			if (!generatedLead) {
				if (noLeadPokemon.includes(species.baseSpecies)) {
					...
					set = this.randomSet(species,pokemonPool, teamDetails, false, isDoubles);
					pokemon.push(set);
				} else {
					set = this.randomSet(species,pokemonPool, teamDetails, true, isDoubles);
					pokemon.unshift(set);
					generatedLead = true;
				}
			} else {
	...
	randomHCTeam(): PokemonSet[] {
		...
		for (const forme of randomN) {
			...
			// Random shininess
			const shiny = this.randomChance(1, 1024);
			const allspecies=this.dex.species.all().filter(x=>x.tier!=='Unreleased'&&x.tier!=='Illegal'&&x.tier!=='CAP'&&x.tier!=='CAP NFE'&&x.tier!=='CAP LC'&&x.tier!=='LC'&&x.tier!=='NFE')
			const set: PokemonSet = {
				name: this.prng.sample(allspecies).name,
				species: species.name,
				gender: species.gender,
				item,
				ability,
