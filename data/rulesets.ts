...
export const Formats: {[k: string]: FormatData} = {
	...
	spikemuthcupgbu: {
		effectType: 'ValidatorRule',
		name: 'Spikemuth Cup GBU',
		desc: "The standard ruleset for the Spikemuth Cup competition.",
		...
		onValidateSet(set, format) {
			...
			if (
				!galarDex.includes(species.baseSpecies) && !galarDex.includes(species.name) &&
			!ioaDex.includes(species.baseSpecies) && !ioaDex.includes(species.name) &&
			!tundraDex.includes(species.baseSpecies) && !tundraDex.includes(species.name)
			) {
				return [`${species.baseSpecies} is banned in ${format.name}.`];
			}
		},
	},
	standarddoubles: {
