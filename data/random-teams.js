...
class RandomTeams extends Dex.ModdedDex {
	...
	queryMoves(moves, hasType = {}, hasAbility = {}, movePool = []) {
		...
		let NoStab = [
			'aquajet', 'bounce', 'explosion', 'fakeout', 'firstimpression', 'flamecharge', 'iceshard', 'pluck', 'pursuit', 'quickattack', 'selfdestruct', 'suckerpunch', 'watershuriken',
	...
	randomSet(template, teamDetails = {}, isLead = false, isDoubles = false) {
		...
		do {
			...
			for (const [k, moveId] of moves.entries()) {
				...
				case 'lavaplume':
					...
					break;
				case 'overheat':
					if (hasMove['fireblast'] || hasMove['lavaplume']) rejected = true;
					break;
				...
				if (!rejected && (counter['physicalsetup'] + counter['specialsetup'] < 2 && (!counter.setupType || counter.setupType === 'Mixed' || (move.category !== counter.setupType && move.category !== 'Status') || counter[counter.setupType] + counter.Status > 3)) &&
					...
					(hasType['Rock'] && !counter['Rock'] && (template.baseStats.atk >= 105 || hasAbility['Rock Head'] || counter.setupType === 'Physical')) ||
					(((hasType['Steel'] && !counter['Steel'] && (hasAbility['Technician'] || hasMove['trickroom'])) || hasAbility['Steelworker'])) ||
		...
		// First, the extra high-priority items
			...
			item = 'Groundium Z';
		} else if (hasMove['electricterrain'] || hasAbility['Electric Surge'] && hasMove['thunderbolt']) {
			item = 'Electrium Z';
	...
	randomFormatsTeam() {
		let teams = {
			...
			gen4ou: [
				...
				'Bronzong||damprock||raindance,hypnosis,stealthrock,explosion|Impish|252,152,8,,96,|||||]Cresselia||lightclay||psychic,thunderwave,raindance,lunardance|Bold|252,,148,,40,68||,0,,,,|||]Kingdra||choicespecs||hydropump,surf,dracometeor,dragonpulse|Timid|32,,,252,,224||,0,,,,|||]Qwilfish||lifeorb|1|waterfall,poisonjab,swordsdance,explosion|Adamant|,252,4,,,252|||||]Heracross||choiceband|1|closecombat,megahorn,stoneedge,sleeptalk|Jolly|4,252,,,,252|||||]Metagross||lumberry||meteormash,earthquake,earthquake,agility|Jolly|60,252,,,,196|||||',
